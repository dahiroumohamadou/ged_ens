...
namespace backend.Repository.Implementations
{
    public class DocumentRepo : IDocument
    {
        ...
        {
           int res=-1;
           var d=_ctx.Documents.Where(dc=>dc.Id==id).FirstOrDefault() ?? null;
            if (d!=null)
            {
                _ctx.Documents.Remove(d);
                _ctx.SaveChanges();
                res = d.Id;
            }
            return res;
        ...
        {
            int res = -1;
            var d=_ctx.Documents.Where(dc=>dc.Id == document.Id).FirstOrDefault() ?? null;
            if (d != null)
            {
        ...
        {
            var arr = _ctx.Documents.Where(dc => (dc.Source == source && dc.Numero == numero & dc.DateSign == dateSign && dc.AnneeAcademique == anneeAca && dc.CycleId == cycleId)).FirstOrDefault() ?? null;
            return arr;
        }

        public Doc ExisteCrp(string source, string numero, string dateSign, string session, string anneeAca, int cycleId)
        {
            var crp = _ctx.Documents.Where(dc => (dc.Source == source && dc.Numero == numero & dc.DateSign == dateSign && dc.Session == session && dc.AnneeAcademique == anneeAca && dc.CycleId == cycleId)).FirstOrDefault() ?? null;
            return crp;
        }

        public Doc ExisteOthers(string source, string numero, string dateSign)
        {
            var arr = _ctx.Documents.Where(dc => (dc.Source == source && dc.Numero == numero & dc.DateSign == dateSign)).FirstOrDefault() ?? null;
            return arr;
        }
        public Doc ExistePv(string source, string session, string promotion, string anneeSortie, int cycleId, int filiereId)
        {
           var pv = _ctx.Documents.Where(dc => (dc.Source == source && dc.Session == session && dc.Promotion == promotion & dc.AnneeSortie == anneeSortie && dc.CycleId == cycleId && dc.FiliereId == filiereId)).FirstOrDefault() ?? null;
           return pv;
        }
        public ICollection<Doc> GetAllByAnnee(string annee)
        {
            var docs = _ctx.Documents
                .Include(d=>d.Cycle)
                .Include(d=>d.Filiere)
                .Where(d => d.DateSign.Substring(6) == annee || d.AnneeSortie.ToString()==annee);
            return docs.ToList();
        }
